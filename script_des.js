...
const hexToBin = (hex) => ('00000000' + parseInt(hex, 16).toString(2)).substr(-8);          //Transforma o unidade hexadecimal para binário
const decToBin = (dec) => ('0000' + parseInt(dec, 10).toString(2)).substr(-4);              //Transforma o unidade decimal para binário
const binToHex = (bin) => parseInt(bin, 2).toString(16);                                    //Transforma o unidade binário para hexadecimal
const bin = (key) =>                                                                        //Transforma o bloco em binário
    chunkString(key, 2)
        .map((hex) => hexToBin(hex))
        .join('');
const shiftString = (str, shift) => str.slice(shift, str.length) + str.slice(0, shift);     //Função que define o processo de "shift" no bloco

const keySchedule = (key) => {                                                              //Função que realiza os "shifts" no bloco
    let subkeys = [];
...
const expandBlock = (block) => E.map((index) => block[index - 1]).join('');                 //Função que expande o bloco

const stringXOR = (str1, str2, len) => {
    let xor = Array(len);                                                    //Função que realiza a operação de XOR com a chave obtida do sBoxOutput
    for (let i = 0; i < len; i++) {
...
const sBoxOutput = (bits) => {                                                              //Função define a chave do passo atual com a chave geral e as informações do bloco da esquerda 
...
const des = (msg, key, subkeys) => {                                                        //Função que realiza o DES
    let perm = IP.map((index) => msg[index - 1]).join('');                                  //Permutação da tabela inicial (IP)
    let L0 = perm.substr(0, perm.length / 2);                                               //Separa o bloco da esqueda
    let R0 = perm.substr(perm.length / 2);                                                  //Separa o bloco da direita

    let prevL0 = L0,
        prevR0 = R0;
    for (let i = 0; i < 16; i++) {                                                          //Realiza a operação de XOR na coluna da Esquerda e inverte com a posição da coluna da Direita
        L0 = prevR0;
...
const encode = (msg, key) => des(msg, key, keySchedule(key));                               //Função que codifica a mensagem
const decode = (msg, key) => des(msg, key, keySchedule(key).reverse());                     //Função que decodifica a mensagem
